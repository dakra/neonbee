...
class DeployableTest extends NeonBeeTestBase {
    ...
    void deployTestUnit(VertxTestContext testContext) {
        ...
        deployable.deploy(vertxMock, CORRELATION_ID).future().onComplete(testContext.failing(t -> testContext.verify(() -> {
    ...
    void deployTestComponent(Vertx vertx, VertxTestContext testContext)
            throws IOException {
        ...
        }).compose(pendingDeployments -> Future.<Message<String>>future(fut -> vertx.eventBus().request(addressA, "", fut))).compose(response -> {
            testContext.verify(() -> {
